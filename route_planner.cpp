...
bool CompareHeuristicAndGValue(RouteModel::Node *node1, RouteModel::Node *node2) {
    return (node1->h_value + node1->g_value) > (node2->h_value + node2->g_value);
}

RouteModel::Node *RoutePlanner::NextNode() {
	std::sort(open_list.begin(), open_list.end(), CompareHeuristicAndGValue);
...
std::vector<RouteModel::Node> RoutePlanner::ConstructFinalPath(RouteModel::Node *current_node) {
    ...
    std::vector<RouteModel::Node> path_found;
  	...
  	auto holder_node = current_node;
  	auto holder_node_parents = nullptr;

    // TODO: Implement your solution here.
  	while(holder_node->parent != nullptr){
      distance += holder_node->distance(*holder_node->parent);
  	...
  	while(!open_list.empty()){
    	auto next_step = NextNode();
      	...
      	if(next_step == end_node){
          	m_Model.path = ConstructFinalPath(next_step);
          	break;
        }
  	}
